...
> **从零开始，手把手教你构建现代化的Web3前端应用**

## 🎯 学习目标

通过这个教程，你将学会：
- ✅ React 19 + TypeScript + Vite 现代前端开发
- ✅ Wagmi v2 + Viem v2 Web3集成实战
- ✅ 钱包连接和网络管理
- ✅ 智能合约交互和状态管理
- ✅ 现代化UI设计和用户体验优化
- ✅ 性能优化和部署策略
...
export const CONTRACT_ADDRESSES: Record<number, { redPacketSystem: Address }> = {
  // Sepolia测试网
  [sepolia.id]: {
    redPacketSystem: '0x9986ec8e9D4d0724e7CD9320eD703a3CD69389C1',
  },
...
/**
 * 红包系统合约ABI（简化版本，实际使用应该从artifacts导入）
 */
...
**创建 `src/hooks/web3/useWeb3Config.ts`**:
...
export const wagmiConfig = createConfig({
  ...
  connectors: [
    ...
    injected(),
    walletConnect({
      projectId: 'your_walletconnect_project_id', // 替换为实际的项目ID
    }),
  ],
  transports: {
    [sepolia.id]: http(),
...
**创建 `src/types/contracts/RedPacketSystem.ts`**:
...
// 合约配置
export const RED_PACKET_ADDRESS = '0x9986ec8e9D4d0724e7CD9320eD703a3CD69389C1' as const
